  ...
  $.fn.duration_picker = function (options) {
    ...
    this.each(function (i, e) {
      ...
      function picker_changed() {
        ...
        e2.find('#jdp-hours').change();
        minutes = parseInt(minute_input.val());
        seconds = parseInt(second_input.val());
        var curStarttimeNum = calcTime($("#time-display-1").text());
        var newDuration = hours*60+minutes;
        var maxDuration = 24*60-parseInt(curStarttimeNum);
      ...
      if (!disabled) {
        ...
        var num_input = $('<input style="width: 30px;" type="number" min="0" value="0">');
        var day_input = num_input.clone().change(picker_changed);
                ...
                var hour_input = num_input.clone().attr('max', 23).change(picker_changed).addClass("hour");
                $('<div class="ui small transparent input"> ' + langs[settings.lang]['hours'] + '</div>').prepend(hour_input).appendTo(picker);
                var minute_input = num_input.clone().attr('max', 59).attr('step', 5).change(picker_changed).addClass("minute");
